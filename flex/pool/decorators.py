...
def deploy_server_model(func):
    __inspect_arguments(func=func)
    @functools.wraps(func)
    def _deploy_model_(server_flex_model:FlexModel, clients_flex_models:List[FlexModel], *args, **kwargs):
...
def collect_clients_weights(func):
    __inspect_arguments(func=func)
    @functools.wraps(func)
    def _collect_weights_(aggregator_flex_model:FlexModel, clients_flex_models:List[FlexModel], *args, **kwargs):
...
def set_aggregated_weights(func):
    ...
    def _deploy_aggregated_weights_(
        aggregator_flex_model:FlexModel, servers_flex_models:FlexModel, *args, **kwargs
    ):
