...
class FLEXibleDatasets:
    ...
    __module_names = [
        "flex.datasets.federated_datasets",
        "flex.datasets.standard_datasets",
    ]
    instances = {
        str(name).lower(): val
        for module in __module_names
        for name, val in import_module(module).__dict__.items()
    }
    __create_key = object()
...
def load(
    name: str = ".", out_dir: str = ".", split=None, return_test: bool = None, **kwargs
):
    """
    ...
    return FLEXibleDatasets.instances[name](
        out_dir=out_dir, split=split, return_test=return_test, **kwargs
    )
