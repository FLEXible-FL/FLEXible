...
from flex.data.flex_dataset import FlexClientDataset, FlexDataset
...
class TestFlexClientDataset(unittest.TestCase):
    ...
    def test_X_data_property_setter(self):
        X_data = np.random.rand(100).reshape([20, 5])
        fcd = FlexClientDataset(X_data=X_data)
        assert np.array_equal(X_data, fcd.X_data)
        X_data = np.random.rand(100).reshape([20, 5])
    ...
    def test_y_data_property_setter(self):
        X_data = np.random.rand(100).reshape([20, 5])
        y_data = np.random.choice(2, 20)
        fcd = FlexClientDataset(X_data=X_data, y_data=y_data)
        assert np.array_equal(y_data, fcd.y_data)
    ...
    def test_X_names_property_valid_X_names(self):
        ...
        fcd = FlexClientDataset(X_data=X_data, y_data=y_data, X_names=X_names)
    ...
    def test_y_names_property_valid_y_names(self):
        ...
        fcd = FlexClientDataset(X_data=X_data, y_data=y_data, y_names=y_names)
        assert np.array_equal(y_data, fcd.y_data)
