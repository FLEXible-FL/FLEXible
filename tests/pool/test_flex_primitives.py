...
from flex.pool.flex_primitives import (
...
class TestFlexPoolPrimitives(unittest.TestCase):
    ...
    def _fixture_ag_news_dataset(self):
        ...
        X_columns = 'text'
        label_column = 'label'
        self.f_imdb = FlexDataDistribution.from_config_with_huggingface_dataset(train_data, self.config,
                                                                                X_columns, label_column)
    ...
    def test_primitives_tf(self):
        ...
        assert all(
            np.all(np.equal(p._models[k]["model"].get_weights()[0], reference_model_params[0]))
        ...
        assert np.any(np.not_equal(p._models["server"]["aggregated_weights"][0], reference_model_params[0]))
        ...
        result = p.servers.map(evaluate_server_model_tf, test_data=self.test_data['text'], test_labels=self.test_data['label'])
        print(result)
