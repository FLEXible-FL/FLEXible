"""01_fedavg.py - Federated Averaging (FedAvg) on the FLEX framework.
...
In FLEX:
    1. Server deploys model to clients via @deploy_server_model.
    2. Clients train locally using standard SGD/Adam.
    3. Server collects parameter differences via @collect_clients_weights.
    4. Aggregator averages client updates via flex.pool.fed_avg.
    5. Server updates its parameters via @set_aggregated_weights.
"""
...
def make_synthetic_non_iid_data(
    ...
    for cid in range(num_clients):
        # Skew class probabilities per client (non-IID)
    ...
    # Server test dataset is balanced
    test_features = []
    test_labels = []
...
def local_train(
    ...
    """Standard local client training with SGD."""
...
def run_fedavg(
    ...
    print(f"=== Running FedAvg (Rounds: {rounds}, Clients: {clients}, Device: {device}) ===")
    ...
    for r in range(rounds):
        # 1. Deploy server model to all clients
        pool.servers.map(copy_server_model_to_clients, pool.clients)

        # 2. Local client training
        pool.clients.map(
            local_train, epochs=epochs, batch_size=batch_size, device=device
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="FedAvg Example in FLEX")
